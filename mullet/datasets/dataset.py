...
class DualPhaseMultiSliceInferenceDataset(Dataset):
    def __init__(self, img_path="/data0/wulei/example_3",
                 normalize=MinMaxNormalization,
                 n_ctx=3):
        super().__init__()
        img_p = sitk.ReadImage(glob(os.path.join(img_path, "*_img_p.nii.gz")))
        img_p_array = sitk.GetArrayFromImage(img_p)
        ...
        img_d = sitk.ReadImage(glob(os.path.join(img_path, "*_img_d.nii.gz")))
        img_d_array = sitk.GetArrayFromImage(img_d)

        liver = sitk.ReadImage(glob(os.path.join(img_path, "*_liver_v.nii.gz")))
        liver_array = sitk.GetArrayFromImage(liver)
        ct = np.concatenate([img_p_array, img_a_array, img_v_array, img_d_array], 0)
